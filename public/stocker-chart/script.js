$.ajax({
  ...
  var purchaseCostPerHead =
    parseFloat(response[0].aveStartingWeight) *
    parseFloat(response[0].costPerHeadPer100Pounds) *
    0.01;
  ...
  var sumCostPerHead =
    purchaseCostPerHead +
    parseFloat(response[0].pastureRentPerHead) +
    parseFloat(response[0].vetCostPerHead) +
    parseFloat(response[0].truckTripPerHead);
  console.log("sumCostPerHead: " + sumCostPerHead);
  //
  // 3. Calculate Interest Cost Per Head
  var interestCostPerHead =
    (parseFloat(response[0].interestRate) * 0.01 * sumCostPerHead) / 2;
  ...
  var totalWeightGainPerHead =
    parseFloat(response[0].weightGainPerDay) *
    parseFloat(response[0].numDaysOnPasture);
  //
  // 6. Final Weight Per Head by adding Starting Weight to Total Weight Gain Per Head
  var finalWeightPerHead =
    parseFloat(response[0].aveStartingWeight) + totalWeightGainPerHead;
  ...
  // 8. Total (Sale) Price
  var totalPrice =
    parseFloat(response[0].numHeadsPurchased) * totalSalePricePerHead;
  ...
  for (i = 0; i < 7; i++) {
    ...
    monthlyWeightPerHeadArray[i] =
      parseFloat(response[0].aveStartingWeight) +
      parseFloat(response[0].weightGainPerDay) * [i] * 30;
    monthlyPricePerHeadPer100PoundsArray[i] =
      parseFloat(response[0].costPerHeadPer100Pounds) -
      ([i] / 6) *
        (parseFloat(response[0].costPerHeadPer100Pounds) -
          parseFloat(response[0].pricePerHeadPer100Pounds));
    monthlyPricePerHeadArray[i] =
      monthlyPricePerHeadPer100PoundsArray[i] *
      0.01 *
      monthlyWeightPerHeadArray[i];
    monthlyTotalPriceArray[i] =
      parseFloat(response[0].numHeadsPurchased) * monthlyPricePerHeadArray[i];
  }
  ...
  console.log(
    "monthlyPricePerHeadPer100PoundsArray: " +
      monthlyPricePerHeadPer100PoundsArray
  );
